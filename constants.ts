...
import { Scenario, ModelConfig, VoiceConfig, UserSettings, Role, ThemeColor } from './types';

export const THEME_CONFIG: Record<ThemeColor, { border: string; bg: string; badge: string }> = {
  blue: {
...
export const AVAILABLE_VOICES: VoiceConfig[] = [
  { id: 'Puck', name: 'Puck', gender: 'Male', style: 'Natural & Relaxed', icon: '👦' },
  { id: 'Charon', name: 'Charon', gender: 'Male', style: 'Deep & Authoritative', icon: '👨' },
  { id: 'Kore', name: 'Kore', gender: 'Female', style: 'Calm & Soothing', icon: '👩' },
  { id: 'Fenrir', name: 'Fenrir', gender: 'Male', style: 'Energetic', icon: '🧔' },
  { id: 'Zephyr', name: 'Zephyr', gender: 'Female', style: 'Clear & Professional', icon: '👱‍♀️' },
];
...
export const INITIAL_ROLES: Role[] = [
  {
    ...
    title: '妇科主任',
    avatarSeed: 'zhang',
    focusAreas: ['临床指南', '治疗方案', '药物副作用'],
    description: '经验丰富的妇科主任，性格严谨，注重循证医学。对新的治疗方案持审慎态度，关注药物的安全性和有效性数据。做决定时果断但保守。',
    hostility: 40,
    verbosity: 40,
    skepticism: 70,
    systemPromptAddon: 'You are Dr. Zhang, a Director of Gynecology. You are rigorous, evidence-based, and cautious about new treatments. You focus on guidelines and safety data.',
  ...
  },
  {
    ...
    title: '妇科主任',
    avatarSeed: 'li',
    focusAreas: ['患者生活质量', '长期管理', '生育力保护'],
    description: '温和儒雅的妇科专家，非常关注患者的长期生活质量和心理状态。在治疗方案的选择上，倾向于能够平衡疗效和患者体验的方案。',
    hostility: 20,
    verbosity: 60,
    skepticism: 40,
    systemPromptAddon: 'You are Dr. Li, a Director of Gynecology. You are gentle, patient-centric, and focused on long-term management and fertility preservation.',
    lastUpdated: new Date().toISOString()
  }
];
...
请按照以上设定进行角色扮演，保持你作为资深医生的专业性和对新信息开放的态度。`,
    knowledgePoints: `### 1. 痛经与子宫内膜异位症概述
...
*   **禁忌**：活动性静脉血栓栓塞疾病；当前或既往动脉及心血管疾病；出现血管病变的糖尿病；当前或既往有肝功能值未恢复正常的重度肝病；当前或既往肝肿瘤；已知或疑似性激素依赖性恶性肿瘤；原因不明的阴道出血；对活性物质或辅料过敏者。
...
*   **内异症预防**：治疗关口前移，早诊断、早发现、早治疗。通过短效COC预防、规范手术操作、尽早临床诊断并药物治疗等措施，减少病灶形成，控制疾病进展，避免或推迟手术。`,
    scoringCriteria: `### 1. 开场 (iOpen) - 满分10分
