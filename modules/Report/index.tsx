...
export const ReportModule: React.FC<ReportModuleProps> = ({ data, onExit, onRetry }) => {
  ...
  const handleReEvaluate = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        addLog("Error: API Key is missing", 'error');
      ...
      addLog('构建 Prompt 完成 (含完整对话记录)', 'info', prompt);

      const ai = new GoogleGenAI({ apiKey });
      addLog('发起流式请求 (Streaming Request)...', 'info');
      ...
      for await (const chunk of responseStream) {
         ...
         if (text) {
            fullText += text;
            chunkCount++;
            addLog(`收到数据块 #${chunkCount}`, 'stream', text);
         }
      }
      
      addLog('流式传输结束。完整响应:', 'info', fullText);
