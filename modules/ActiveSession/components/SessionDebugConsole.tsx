...
export const SessionDebugConsole: React.FC<Props> = ({ logs, onClear }) => {
  ...
  // Role-based log filtering: non-admin users only see transcript logs
  const visibleLogs = isAdmin ? logs : logs.filter(log => log.category === 'transcript');
    ...
    >
      ...
      >
          ...
          <div className="flex items-center">
            ...
            {!isExpanded && visibleLogs.length > 0 && (
              <div className="ml-3 flex items-center text-[10px] bg-white px-2 py-0.5 rounded-full border border-slate-200 shadow-sm max-w-[200px]">
                <div className={`w-1.5 h-1.5 rounded-full mr-1.5 flex-shrink-0 ${visibleLogs[visibleLogs.length - 1].type === 'error' ? 'bg-red-500' : 'bg-green-500'
                  }`}></div>
                <span className="truncate text-slate-600 font-sans">
                  {visibleLogs[visibleLogs.length - 1].message}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{visibleLogs.length} events</span>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5 scroll-smooth bg-white" ref={scrollRef}>
        {visibleLogs.map((log, i) => (
          <div key={i} className="flex items-start space-x-2 border-b border-slate-50 pb-1.5 last:border-0 group">
            ...
            <span className={`break-all leading-tight ${log.type === 'error' ? 'text-red-600 bg-red-50 px-1 rounded' :
        ...
        {visibleLogs.length === 0 && isExpanded && (
