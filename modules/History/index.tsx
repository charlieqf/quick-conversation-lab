...
export const HistoryModule: React.FC<HistoryModuleProps> = ({ userProfile, onUpdateProfile, onViewReport, onResume }) => {
  ...
  // Load Data
  useEffect(() => {
    const loadedSessions = JSON.parse(localStorage.getItem('quick_sessions') || '[]');
    // Sort by latest
    setSessions(loadedSessions.sort((a: any, b: any) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()));
  ...
  const handleDeleteOne = (id: string) => {
     if (window.confirm("确定要删除这条记录吗？")) {
        ...
        localStorage.setItem('quick_sessions', JSON.stringify(newSessions));
     }
  };
  ...
  const confirmBatchDelete = () => {
     const newSessions = sessions.filter(s => !selectedIds.has(s.id));
     setSessions(newSessions);
     localStorage.setItem('quick_sessions', JSON.stringify(newSessions));
